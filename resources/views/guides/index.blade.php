...
@section('title', 'Energy switching guides — Switchly')
@section('meta_description', 'Free UK energy guides from Switchly: how to switch supplier, tariffs explained, and practical ways to cut your bills.')
...
@section('content')
    <section class="bg-[#f3f3f3] py-14 sm:py-20">
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
            ...
            <div class="mx-auto mt-12 grid max-w-5xl gap-5 sm:grid-cols-1 lg:grid-cols-3 lg:gap-6">
                @foreach ($guides as $slug => $guide)
                    <article class="flex flex-col rounded-[1.25rem] bg-white p-7 shadow-[0_10px_40px_rgba(0,0,0,0.06)] sm:p-8">
                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#e8f6b8] text-sm font-extrabold text-switchly-ink">
                            {{ $loop->iteration }}
                        </span>
                        <h2 class="mt-5 text-lg font-bold tracking-tight text-switchly-ink sm:text-xl">
                            <a href="{{ route('guides.show', $slug) }}" class="transition hover:text-switchly-ink">
                                {{ $guide['title'] }}
                            </a>
                        </h2>
                        <p class="mt-2 flex-1 text-sm leading-relaxed text-switchly-muted">
                            {{ $guide['description'] }}
                        </p>
                        ...
                        <a
                            ...
                            class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-switchly-ink transition hover:text-switchly-lime"
                        >
                            Read guide
            ...
            <div class="mx-auto mt-14 max-w-5xl overflow-hidden rounded-[1.5rem] bg-switchly-black px-6 py-10 text-center sm:px-10 sm:py-12">
