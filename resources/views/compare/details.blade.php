...
@section('title', 'Your details — Switchly')
...
@section('content')
    ...
    <main class="mx-auto max-w-xl px-5 pb-16 sm:px-8">
        <div class="text-center">
            <h1 class="text-2xl font-extrabold tracking-tight text-switchly-ink sm:text-3xl lg:text-[2rem]">
                Let’s find your best energy deals
            </h1>
            <p class="mt-2 text-sm text-switchly-muted sm:text-base">
        ...
        <form action="{{ route('compare.details.store') }}" method="POST" class="mt-8 space-y-4">
            ...
            {{-- 1. Postcode --}}
            <section class="rounded-2xl border border-switchly-border bg-white p-5 sm:p-6">
                <h2 class="text-base font-bold text-switchly-ink sm:text-[1.05rem]">
                    1. Where do you live?
                </h2>
                <div class="mt-4">
                    <x-form.input
                ...
                <a href="#" class="mt-3 inline-block text-sm font-medium text-switchly-lime underline underline-offset-2 hover:brightness-90">
            ...
            <div class="flex items-start gap-3 rounded-2xl border border-[#d4e8a0] bg-[#f4f9e0] px-4 py-3.5">
                <svg class="mt-0.5 h-5 w-5 shrink-0 text-switchly-lime" viewBox="0 0 20 20" fill="none" aria-hidden="true">
                ...
                <p class="text-sm leading-relaxed text-switchly-muted">
            ...
            <button
                type="submit"
                class="inline-flex w-full items-center justify-between rounded-full bg-switchly-black py-3.5 pl-6 pr-2.5 text-base font-semibold text-white transition hover:bg-neutral-800"
            >
