...
>
    <div class="mx-auto grid h-[4.75rem] max-w-7xl grid-cols-[1fr_auto] items-center gap-4 px-5 sm:px-8 lg:grid-cols-[1fr_auto_1fr] lg:gap-6 lg:px-10">
        ...
        <nav class="hidden items-center gap-8 lg:flex xl:gap-10" aria-label="Primary">
            @if ($variant === 'secure')
                <a href="{{ route('how-it-works') }}" class="text-[0.9375rem] font-medium text-switchly-muted transition hover:text-switchly-ink">
                    How it works
                </a>
            ...
            <div class="relative" @keydown.escape.window="helpOpen = false">
                ...
                >
                    ...
                    <a href="{{ route('contact') }}" class="block px-4 py-2.5 text-sm font-medium text-switchly-muted hover:bg-gray-50 hover:text-switchly-ink">Contact us</a>
                    <a href="{{ route('guides.index') }}" class="block px-4 py-2.5 text-sm font-medium text-switchly-muted hover:bg-gray-50 hover:text-switchly-ink">Guides</a>
                </div>
            </div>
        </nav>
        ...
        <div class="hidden justify-self-end lg:block">
            @if ($variant === 'secure')
                <div class="flex items-center gap-3 rounded-xl border border-switchly-border bg-white px-3.5 py-2 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                    <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#eaf8c4] text-switchly-lime">
                    ...
                    <div class="min-w-0 leading-tight">
                        <p class="text-sm font-bold text-switchly-ink">Your data is safe</p>
                        <p class="mt-0.5 text-xs text-switchly-muted">We never share your personal information.</p>
                    </div>
                </div>
            @else
                <a
    ...
    >
        <div class="flex h-[4.75rem] shrink-0 items-center justify-between border-b border-switchly-border px-5 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center" aria-label="Switchly home" @click="close()">
                <img
                    src="{{ asset('images/logo.png') }}"
                    alt="Switchly"
                    class="h-8 w-auto"
            ...
            <button
                type="button"
                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-switchly-border text-switchly-ink"
                @click="close()"
        ...
        <nav class="flex flex-1 flex-col overflow-y-auto px-5 py-6 sm:px-8" aria-label="Mobile">
            @if ($variant === 'secure')
                <a href="{{ route('how-it-works') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">How it works</a>
                <a href="{{ url('/#why-switchly') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">About us</a>
                <a href="{{ url('/#faqs') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Help</a>
                <div class="mt-8 flex items-center gap-3 rounded-xl border border-switchly-border bg-[#f8f8f8] px-4 py-4">
                    <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#eaf8c4] text-switchly-lime">
                        <svg class="h-4 w-4" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                            <path d="M4 7V5.5a4 4 0 1 1 8 0V7" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" />
                            <rect x="2.75" y="7" width="10.5" height="7.25" rx="2" stroke="currentColor" stroke-width="1.6" />
                        </svg>
            ...
            @else
                <a href="{{ route('how-it-works') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">How it works</a>
                <a href="{{ url('/#why-switchly') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Why Switchly</a>
                <a href="{{ route('compare.details') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Compare</a>
                <a href="{{ url('/#faqs') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Help</a>
                <a href="{{ route('contact') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Contact</a>
                <a href="{{ route('guides.index') }}" class="border-b border-switchly-border py-4 text-lg font-semibold text-switchly-ink" @click="close()">Guides</a>

                <div class="mt-auto pt-8">
                    <a
                        href="{{ route('compare.details') }}"
                        class="inline-flex w-full items-center justify-between gap-2.5 rounded-full bg-switchly-black py-3.5 pl-5 pr-2.5 text-base font-semibold text-white"
                        @click="close()"
                    >
                        Check my rates
