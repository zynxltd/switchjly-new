...
<nav aria-label="Progress" class="mx-auto w-full max-w-md px-5 py-8 sm:px-8 sm:py-10">
    <ol class="relative flex items-start justify-between">
        <li
            class="pointer-events-none absolute top-4 right-[16%] left-[16%] h-px bg-switchly-border"
            aria-hidden="true"
        >
            <span
                class="absolute inset-y-0 left-0 bg-switchly-lime transition-all"
        ...
        @foreach ($steps as $number => $label)
            ...
            <li class="relative z-10 flex w-28 flex-col items-center text-center">
                <span @class([
                    ...
                    'bg-switchly-lime text-white' => $active || $done,
                ...
                <span @class([
                    ...
                    'font-bold text-switchly-ink' => $active,
