<section class="bg-white py-12 sm:py-14" aria-label="Energy suppliers we compare">
    <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        <h2 class="text-center text-lg font-extrabold tracking-tight text-switchly-ink sm:text-xl">
        ...
        <div class="mt-8 -mx-5 sm:mx-0 sm:mt-10 sm:hidden">
            <ul class="flex snap-x snap-mandatory items-center gap-8 overflow-x-auto px-5 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                @foreach ([
                    ['octopus-color.png', 'Octopus Energy', 'h-7'],
                    ['eon-color.png', 'E.ON', 'h-6'],
                    ['scottishpower-color.png', 'ScottishPower', 'h-7'],
                    ['british-gas-color.png', 'British Gas', 'h-6'],
                    ['ovo-color.png', 'OVO Energy', 'h-6'],
                    ['utilita-color.png', 'Utilita', 'h-6'],
                ] as [$file, $alt, $height])
                    <li class="flex shrink-0 snap-start items-center justify-center">
                        <img
                            src="{{ asset("images/suppliers/{$file}") }}"
                            alt="{{ $alt }}"
                            class="{{ $height }} w-auto max-w-[6.5rem] object-contain grayscale"
                        >
                    </li>
                @endforeach
            </ul>
