...
<section id="why-switchly" class="bg-[#f3f3f3] py-20 sm:py-24" aria-labelledby="why-heading">
    <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        ...
        <div class="mt-12 grid gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4 lg:gap-6 xl:gap-7">
            @foreach ($features as $feature)
                <article class="rounded-[1.25rem] bg-white p-8 shadow-[0_10px_40px_rgba(0,0,0,0.06)] sm:p-9 lg:min-h-[17.5rem]">
                    <div class="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#e8f6b8] text-switchly-ink">
                        @if ($feature['icon'] === 'tag')
                    ...
                    <h3 class="text-xl font-bold tracking-tight text-switchly-ink">
                        {{ $feature['title'] }}
                    </h3>
                    <p class="mt-3 text-[0.9375rem] leading-relaxed text-switchly-muted">
                        {{ $feature['body'] }}
                    </p>
                </article>
            @endforeach
