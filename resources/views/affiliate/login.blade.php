...
<head>
    ...
    <title>Affiliate login — Switchly</title>
    @fonts
...
<body class="min-h-screen bg-[#f3f3f3] font-sans text-switchly-ink antialiased">
    <div class="mx-auto flex min-h-screen max-w-md flex-col justify-center px-5 py-12">
        <div class="rounded-[1.5rem] bg-white p-8 shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
            ...
            <p class="mt-5 text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-switchly-lime">Affiliate</p>
            <h1 class="mt-2 text-2xl font-extrabold tracking-tight">Partner sign in</h1>
            <p class="mt-1 text-sm text-switchly-muted">Track referrals, clicks, and estimated earnings.</p>
            ...
            <form method="post" action="{{ route('affiliate.login.store') }}" class="mt-6 space-y-4">
                ...
                <button type="submit" class="inline-flex w-full items-center justify-center rounded-full bg-switchly-lime py-3.5 text-sm font-bold text-switchly-ink hover:brightness-95">
                    Sign in
                </button>
            </form>
        </div>
