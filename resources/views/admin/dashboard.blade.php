...
@section('content')
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ([
            ...
            <div class="rounded-[1.25rem] bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
                <p class="text-xs font-medium text-switchly-muted">{{ $stat['label'] }}</p>
                <p class="mt-2 text-2xl font-extrabold tracking-tight">{{ $stat['value'] }}</p>
            </div>
        @endforeach
    </div>

    <div class="mt-4 rounded-[1.25rem] bg-switchly-black p-5 text-white sm:p-6">
        <p class="text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-switchly-lime">Payouts</p>
    ...
    <div class="mt-8 overflow-hidden rounded-[1.25rem] bg-white shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
        <div class="flex items-center justify-between border-b border-switchly-border px-5 py-4">
            <h2 class="text-base font-bold">Recent leads</h2>
            <a href="{{ route('admin.leads') }}" class="text-sm font-semibold text-switchly-ink hover:text-switchly-lime">View all</a>
        </div>
                ...
                <tbody>
                    @forelse ($recentLeads as $lead)
                        <tr class="border-t border-switchly-border">
                            ...
                            <td class="px-5 py-3 text-switchly-muted">{{ $lead->created_at?->diffForHumans() }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-8 text-center text-switchly-muted">No leads yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
