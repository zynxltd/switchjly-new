...
<head>
    ...
    <title>Admin login — Brillia</title>
    @fonts
...
<body class="min-h-screen bg-[#f3f3f3] font-sans text-brillia-ink antialiased">
    <div class="mx-auto flex min-h-screen max-w-md flex-col justify-center px-5 py-12">
        <div class="rounded-[1.5rem] bg-white p-8 shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
            ...
            <p class="mt-5 text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-brillia-lime">Admin</p>
            <h1 class="mt-2 text-2xl font-extrabold tracking-tight">Sign in</h1>
            <p class="mt-1 text-sm text-brillia-muted">Manage leads, affiliates, and payouts.</p>
            ...
            <form method="post" action="{{ route('admin.login.store') }}" class="mt-6 space-y-4">
                ...
                <button type="submit" class="inline-flex w-full items-center justify-center rounded-full bg-brillia-black py-3.5 text-sm font-semibold text-white hover:bg-neutral-800">
                    Sign in
                </button>
            </form>
        </div>
